    ...
    def execute(
        ...
        **kwargs
        ...
        try:
            ...
            hematology_entries = self.hematology_retriever.retrieve_entries_for_correction(
                transcription_id=transcription_id,
                transcription_text=whisper_text,
                top_k_queries=top_k_queries,
            ...
            if medical_docs:
                ...
                context_parts.extend([f"  - {doc[:200]}..." if len(doc) > 200 else f"  - {doc}" for doc in medical_docs[:3]])
            
            if hematology_entries:
                context_parts.append("\n血液學詞典參考：")
                context_parts.extend([f"  - {entry[:200]}..." if len(entry) > 200 else f"  - {entry}" for entry in hematology_entries[:3]])
