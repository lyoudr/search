...
class QualityEvaluator:
    ...
    def evaluate_correction_quality(
        ...
        llm_evaluation = self._evaluate_with_llm(
            corrected_text=corrected_text, model_name=model_name
        )
        ...
        if chinese_typos:
            ...
            score -= penalty
            issues.append(
                f"Detected {len(chinese_typos)} Chinese typo(s): {', '.join(chinese_typos[:3])}"
            )
        ...
        if english_typos:
            ...
            score -= penalty
            issues.append(
                f"Detected {len(english_typos)} English medical term typo(s): {', '.join(english_typos[:3])}"
            )
    ...
    def _evaluate_with_llm(
        self, corrected_text: str, model_name: str = "gpt-5.2"
    ) -> Dict[str, Any]:
        """
    ...
    def should_try_alternative(
        self, quality_assessment: Dict[str, Any], methods_tried: list
    ) -> bool:
        """
    ...
    def suggest_next_method(
        self, current_method: str, methods_tried: list, transcription_text: str
    ) -> Optional[str]:
        """
        ...
        if (
            "direct_llm" in methods_tried
            and "medical_document_rag" not in methods_tried
        ):
            return "medical_document_rag"

        if (
            "medical_document_rag" in methods_tried
            and "hematology_rag" not in methods_tried
        ):
            # Check if text seems hematology-related
            if any(
                re.search(pattern, transcription_text)
