...
class ModelManager:
    ...
    def __init__(self):
        # OpenAI client for OpenAI API models (gpt-5, gpt-4, etc.)
        ...
        #         base_url="https://router.huggingface.co/v1", api_key=settings.HF_TOKEN
        #     )
    ...
    def download_hf_model(
        self, model_config: ModelConfig, cache_dir: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if model_config.load_in_4bit:
                ...
                if not BITSANDBYTES_AVAILABLE:
                    raise ImportError(
                        "bitsandbytes is required for 8-bit quantization. Install it with: pip install bitsandbytes"
                    )
            ...
            model = AutoModelForCausalLM.from_pretrained(
                model_config.hf_model_id, **model_kwargs
            )
    ...
    def load_model(self, model_name: str, force_reload: bool = False):
        ...
        if config.provider == "huggingface" and config.hf_model_id:
            try:
                ...
                # Load model
                model = AutoModelForCausalLM.from_pretrained(
                    config.hf_model_id, **model_kwargs
                )
    ...
    def generate_text(
        ...
        **kwargs,
        ...
        if config.provider == "openai" and config.model_type == "api":
            ...
            # Keep backward compatibility for existing callers that still pass max_tokens.
            model_name = (config.api_model_name or "").lower()
            ...
            if is_gpt5_family:
                response = self.openai_client.chat.completions.create(
                    ...
                    # GPT-5 currently only supports default temperature behavior.
                    # Do not send temperature to avoid unsupported_value errors.
        ...
        # Hugging Face local models
            ...
            raise ValueError(
                f"Unsupported model provider/type: {config.provider}/{config.model_type}"
            )
